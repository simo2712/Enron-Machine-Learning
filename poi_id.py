...
def compute_ratio(poi_messages, total_messages):
    ...
    if poi_messages != 'NaN' and total_messages != 'NaN':
        ratio = poi_messages/float(total_messages)
    else:
        ratio = 0
    return ratio
...
for person in my_dataset:
    ...
    my_dataset[person]['ratio_from_poi'] = ratio_from_poi
    ...
    my_dataset[person]['ratio_to_poi'] = ratio_to_poi  
    
new_features = features_list + ['ratio_to_poi', 'ratio_from_poi']
...
for i in sorted_scores[0:10]:
    print i
kbest_features_list = poi_label + list(map(lambda x: x[0], sorted_scores))[0:10]
print kbest_features_list

kbest_features_list.remove('ratio_to_poi')
...
#Create new labels and features with the two new features
data = featureFormat(my_dataset, kbest_features_list+ \
...
#validate_clf(nb_grid_search, new_features, new_labels, nb_param)
